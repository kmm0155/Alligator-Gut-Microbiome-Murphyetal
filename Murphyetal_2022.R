...
#Interaction term is significant here
resultsshmass=lm(SHANNONS~TREAT+WEIGHT+SEX+SEX:WEIGHT,data=datum,na.action=na.omit)
summary(resultsshmass)
plot(SHANNONS~WEIGHT, data=datum)
abline(lm(SHANNONS~WEIGHT, data=datum))
plot(WEIGHT~SEX,data=datum)

resultssh1=lm(SHANNONS~relevel(TREAT,ref = "High")+WEIGHT+SEX+SEX:WEIGHT,data=datum,na.action=na.omit)
summary(resultssh1)

#Interaction term was removed here because it is not significant 
resultsshe2 = lm(SHANNONS~ESTRADIOL+WEIGHT+SEX,data=datum,na.action=na.omit)
summary(resultsshe2)
plot(SHANNONS~ESTRADIOL, data=datum)
abline(lm(SHANNONS~ESTRADIOL, data=datum))

resultsshannonse21 = glm(SHANNONS~E2+WEIGHT+SEX+WEIGHT:SEX,data=datum,na.action=na.omit)
summary(resultsshannonse21)

#Plot for Supplementary Figure 2A
...
#Interaction term was removed here because it was not significant
resultspie=lm(PIE~TREAT+WEIGHT+SEX,data=datum,na.action=na.omit)
summary(resultspie)

resultspie1=lm(PIE~relevel(TREAT,ref = "High")+WEIGHT+SEX,data=datum,na.action=na.omit)
summary(resultspie1)

#Interaction term was removed here because it was not significant 
resultspiee2 = lm(PIE~ESTRADIOL+WEIGHT+SEX,data=datum,na.action=na.omit)
summary(resultspiee2)
plot(PIE~ESTRADIOL, data=datum)
abline(lm(PIE~ESTRADIOL, data=datum))

resultspiee21 = glm(PIE~E2+WEIGHT+SEX,data=datum,na.action=na.omit)
summary(resultspiee21)
...
resultsotu=lm(OT~TREAT+WEIGHT+SEX,data=datum,na.action=na.omit)
summary(resultsotu)
confint(resultsotu)
  
resultsotu1=lm(OT~relevel(TREAT,ref = "High")+WEIGHT+SEX,data=datum,na.action=na.omit)
summary(resultsotu1)

resultsotue2 = lm(OT~ESTRADIOL+WEIGHT+SEX,data=datum,na.action=na.omit)
summary(resultsotue2)
plot(OT~ESTRADIOL, data=datum)

resultsotue21 = glm(OT~E2+WEIGHT+SEX,data=datum,na.action=na.omit)
summary(resultsotue21)
...
summary(results)
plot(ESTRADIOL~WEIGHT, data=datum)
abline(lm(ESTRADIOL~WEIGHT), data=datum)
...
ggplot(datum, aes(x = TREAT_2, y = ESTRADIOL, shape = SEX)) +
  geom_point(size = 3) +
  scale_shape_manual(values = c(1, 2)) +
  theme_classic() +
  labs(x="Treatment", y="Estradiol concentration (pg/mL)")

dev.off()

#Plot for Supplementary Figure 3

tiff("Figure1.tiff", width = 4, height = 4, units = 'in', res = 300)

ggplot(datum, aes(x = WEIGHT, y = ESTRADIOL, shape = SEX, lty = SEX)) +
  geom_point(size = 3) + #aes(color = factor(SEX))) +
  #scale_color_manual(values = c("black", "black"), name = "Sex", labels = c("F", "M")) +
  geom_smooth(method=lm, se = FALSE, size=0.5, color = "black") +
  scale_shape_manual(values = c(1, 0), name= "Sex", labels = c("F", "M")) +
  theme_classic() +
  labs(x="Mass (g)", y="Estradiol concentration (pg/mL)")
